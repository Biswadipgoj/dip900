...
export default function AdminDashboard() {
  ...
  async function selectCustomerFn(customer: Customer) {
    ...
    // Load breakdown — handle errors gracefully
    const { data: bd, error: bdErr } = await sb.rpc('get_due_breakdown', { p_customer_id: customer.id });
    if (bdErr) {
      console.error('get_due_breakdown RPC error:', bdErr);
      // Build a fallback breakdown from EMI data so modal still works
      const emiList = (emis as EMISchedule[]) || [];
      const nextUnpaid = emiList.find(e => e.status === 'UNPAID');
      // Auto-calculate fine from EMI rows
      const autoFine = calculateTotalFineFromEmis(emiList);
      const firstChargeDue = customer.first_emi_charge_paid_at ? 0 : (customer.first_emi_charge_amount || 0);
      const fallback: DueBreakdown = {
        customer_id: customer.id,
        customer_status: customer.status,
        next_emi_no: nextUnpaid?.emi_no,
        next_emi_amount: nextUnpaid?.amount,
        next_emi_due_date: nextUnpaid?.due_date,
        next_emi_status: nextUnpaid?.status,
        fine_due: autoFine,
        first_emi_charge_due: firstChargeDue,
        total_payable: (nextUnpaid?.amount ?? 0) + autoFine + firstChargeDue,
        popup_first_emi_charge: firstChargeDue > 0,
        popup_fine_due: autoFine > 0,
        is_overdue: nextUnpaid ? new Date(nextUnpaid.due_date) < new Date() : false,
      };
  ...
  async function loadReportSummary(filter: 'today' | '7d' | '30d') {
    ...
    try {
      ...
      const { data } = await supabase
        .from('payment_requests')
        .select('total_emi_amount, fine_amount, total_amount')
        .eq('status', 'APPROVED')
        .gte('approved_at', fromDate + 'T00:00:00Z');

      const rows = data || [];
      const emi_collected = rows.reduce((s, r) => s + (Number(r.total_emi_amount) || 0), 0);
      const fine_collected = rows.reduce((s, r) => s + (Number(r.fine_amount) || 0), 0);
      const total = rows.reduce((s, r) => s + (Number(r.total_amount) || 0), 0);
      setReportSummary({ emi_collected, fine_collected, total });
    } finally {
  ...
  async function updateFineSettings() {
    const { error } = await supabase.from('fine_settings').update({
      default_fine_amount: fineSettings.default_fine_amount,
      weekly_fine_increment: fineSettings.weekly_fine_increment,
    }).eq('id', 1);
          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="card p-6">
              ...
              <div className="flex items-end gap-4">
                ...
                <div className="flex-1 max-w-xs">
                  <label className="form-label">Weekly Increment (₹)</label>
                  <input
                    type="number"
              ...
              <p className="text-xs text-ink-muted mt-2">
                Default ₹450. Applies when the next unpaid EMI is past due. Weekly increment: ₹25 per week overdue.
              </p>
            </div>

            {/* Collection Summary Report */}
            <div className="card p-6">
              <p className="section-header">Collection Summary</p>
              <div className="flex gap-2 mb-4">
                {(['today', '7d', '30d'] as const).map(f => (
                  <button
                    key={f}
                    onClick={() => loadReportSummary(f)}
                    className={`px-4 py-2 rounded-lg border text-sm font-medium transition-all ${
                      reportFilter === f && reportSummary
              ...
              {reportSummary && !reportLoading && (
                <div className="grid grid-cols-3 gap-4">
                  <div className="bg-success-light border border-success-border rounded-xl p-4 text-center">
                    <p className="text-xs text-success mb-1">EMI Collected</p>
                    <p className="num text-lg font-bold text-success">{fmt(reportSummary.emi_collected)}</p>
                  </div>
                  <div className="bg-danger-light border border-danger-border rounded-xl p-4 text-center">
                    <p className="text-xs text-danger mb-1">Fine Collected</p>
                    <p className="num text-lg font-bold text-danger">{fmt(reportSummary.fine_collected)}</p>
                  </div>
                  <div className="bg-brand-50 border border-brand-200 rounded-xl p-4 text-center">
                    ...
                    <p className="num text-lg font-bold text-brand-600">{fmt(reportSummary.total)}</p>
                  </div>
                </div>
              )}
            </div>
